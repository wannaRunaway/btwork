...
import com.kulun.energynet.databinding.LayoutPriceDetailsBinding;
...
import com.kulun.energynet.ui.basepopup.PrciePopup;
        ...
        asyncHttpClient.post(url, requestParams, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                if (carinfo.isSuccess()) {
                    ...
                    if (carinfo.getContent().size() == 0 || !carinfo.getContent().get(0).isFlag() || yuyueTime >= dateToStamp(DateUtils.getLastDayOfMonth(startYear, startMonth))) {  //false 表示不可以进行活动
...
//                    Utils.toast(PromotionDetailsActivity.this, carinfo.getMsg()+"");
            ...
            public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
                Toast.makeText(PriceDetailsAcrivity.this, "请求失败", Toast.LENGTH_LONG).show();
            }
        });
    }
...
//                buyPackage();
    ...
    private void clickNext(CarContent carContent) {
        ...
        if (!carContent.isFlag()) {  //false 表示不可以进行活动
            promotionPopup.tv_buy.setClickable(false);
            promotionPopup.tv_buy.setBackground(getResources().getDrawable(R.drawable.promotion_bottom_dark));
            promotionPopup.tv_precaution_add.setText("不可以进行活动");
        } else {
            if (2 - isOnlyStartNextMonth > 1) {
                promotionPopup.tv_precaution_add.setText("提示:购买周期超过了活动时间");
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        Toast.makeText(PriceDetailsAcrivity.this, "支付失败", Toast.LENGTH_SHORT).show();
                    }
                    break;
                }
                case SDK_AUTH_FLAG: {
