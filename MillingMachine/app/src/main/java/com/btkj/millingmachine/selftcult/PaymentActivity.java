...
public class PaymentActivity extends BaseActivity implements View.OnClickListener, TimeFinishInterface {
    ...
    private AlertDialog dialog, vipdialog, wxdialog, alidialog;
    private String aliserino,weixingserino;
//    private ImageView weixingImageView, aliImageView;
    private int weight;
    ...
    private Runnable huiyuanrunnable = new Runnable() {
        @Override
        public void run() {
            getCarUid();
        }
    ...
    private Runnable alipayrunnable = new Runnable() {
        @Override
        public void run() {
            cyclePayRequest(aliserino, "ali");
        }
    };
    private Runnable wxpayrunnable = new Runnable() {
        @Override
        public void run() {
            cyclePayRequest(weixingserino, "wx");
        }
    };
...
//        binding.imgPhone.setOnClickListener(this);
        ...
        binding.weixing.setOnClickListener(this);
        binding.ali.setOnClickListener(this);
        initDialog();
        initvipDialog();
        initwxDialog();
        initaliDialog();
//        loadQrcode(3);//微信支付购米2，支付宝支付购米3
//        loadQrcode(2);//微信支付购米2，支付宝支付购米3
        ...
        httpClient.post(url, requestParams, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                if (aliWxPayModel.isSuccess()) {
                    ...
                    imageView.setImageBitmap(image);
                    switch (type){//微信支付购米2，支付宝支付购米3
                        case 2:
//                            initwxDialog(image);
                            weixingserino = aliWxPayModel.getData().getSerialNo();
                            break;
                        case 3:
//                            initaliDialog(image);
...
//                Utils.toast(PaymentActivity.this, "网络请求异常");
    ...
    private void getCarUid() {
        ...
        if (iRet != 0) {
            handler.postDelayed(huiyuanrunnable, 2000);
...
//                    finish();
    ...
    //订单查询
    private void cyclePayRequest(String aliserino, final String flag) {
        ...
        httpClient.post(url, requestParams, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                String json = new String(responseBody);
                VipPayModel vipPayModel = GsonUtils.getInstance().fromJson(json, VipPayModel.class);
                Utils.logserial(json + "订单查询:" + aliserino);
                if (vipPayModel.isSuccess()) {
                    topayResult(vipPayModel.getData().getAmount(), vipPayModel.getData().getSerialNo());
                } else {
                    switch (flag){
    ...
    private void topayResult(String money, String serialNo) {
        startcult();
        toResult(money, serialNo);
    }

    private void toResult(String money, String serialNo) {
