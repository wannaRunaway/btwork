...
import com.btkj.millingmachine.rechargequery.RechargeLoginCardPhoneActivity;
...
//        super.onSaveInstanceState(outState);
    ...
    private void initCountTimer() {
        timeCount = new TimeCount(60000, 1000, binding.time, this);
        timeCount.start();
    }
        ...
        if (configModel != null) {
            ...
            if (config != null) {
                if (config.getIsShowDeviceLogo() == 0) {
                    Utils.Glide(this, config.getDeviceLogoImgUrl(), binding.imgTopIcon);
                }
                Utils.Glide(this, config.getLogoImgUrl(), binding.bottomIcon);
                binding.title.setText(config.getLogoTitle());
                binding.tvPhone.setText("服务电话：" + config.getServiceCall());
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.img_recharge:
                intent.setClass(MainActivity.this, RechargeLoginCardPhoneActivity.class);
                startActivity(intent);
                break;
