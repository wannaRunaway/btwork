...
//                Utils.snackbar( MainActivity.this, "您已经预约了站点，无法选择其他站点了");
//                return true;
//            }
...
//        }
        ...
        if (contentBean.getStatus().equals("运营中")) {
...
//            if (User.getInstance().isDelay()) {
//                Utils.snackbar(MainActivity.this, "只能延迟一次哦");
//                return;
//            }
...
//                            SharePref.put(MainActivity.this, API.bindcarlist, json.get("bindcarlist").getAsString());
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_right:
                if (stationlist == null) {
                    Utils.snackbar(MainActivity.this, "请等待地图站点刷新");
                    return;
                }
            ...
            case R.id.img_scan:
                ...
                AndPermission.with(this)
                        ...
                        .onDenied(permissions -> {
                            ...
                            Utils.snackbar(MainActivity.this, "用户拒绝权限");
                        })
                        .start();
...
//            }
    ...
    private void inviteAgain(String data) {
        ...
        new MyRequest().myrequest(url, requestParams, MainActivity.this, getApplicationContext(), true, new Myparams() {
            @Override
            public void message(String json) {
                Utils.snackbar(MainActivity.this, "接受邀请成功");
            }
        });
    }
...
//        if (User.getInstance().isYuyue()) {
//            Utils.snackbar( MainActivity.this, "正在预约中，无法选择其他站点");
//            return;
//        }
        if (station == null) {
...
//        //id  [int]	是	预约id
//        if (!User.getInstance().isYuyue()) {
//            Utils.snackbar(MainActivity.this, "请先预约");
//            return;
//        }
