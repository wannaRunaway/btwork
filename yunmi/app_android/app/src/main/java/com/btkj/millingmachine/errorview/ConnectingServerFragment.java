...
import com.btkj.millingmachine.databinding.ActivityConnectingServerBinding;
...
public class ConnectingServerFragment extends Fragment {
    private ActivityConnectingServerBinding binding;
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.activity_connecting_server, null);
        binding = ActivityConnectingServerBinding.bind(view);
        initHeaderBottom();
        return view;
    }
    ...
    public void initHeaderBottom() {
        ...
        if (config != null) {
            if (config.getIsShowDeviceLogo() == 0) {
                Utils.Glide(getContext(), config.getDeviceLogoImgUrl(), binding.topIcon);
            }
